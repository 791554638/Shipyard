...
| **v0.6.0（已发布）** | 个人研发环境部署闭环（kind + 本地 registry，`make dev` 内循环） | **能闭环迭代** |
| v0.6.1 | 版本与发布文档对齐 | 能闭环迭代 |
| **v0.7.0（已发布）** | 本地研发模式：compose override 分层 + 多项目 vhost + `.dockerignore` | **能闭环迭代** |

主线演进：**能跑 → 能验证 → 能安全地跑 → 能闭环迭代**。
"改一次代码，集群里能自动变成新版本吗？"—— v0.6.0 已用 `make dev`（约 40 秒）回答。
下一个自然的问题：**组件能自由换版本、自由组合吗？**（→ v0.7.0）
...
| **v0.7.0** | 本地研发模式（override 分层 + 多项目 vhost） | 内循环 | 开发体验 |
| v0.7.1+ | 组件化与可配置栈 | 配置层 | 工程 |
...
> **v0.7.0 已落地的本地研发模式**（2026-09 发布）：
> * compose override 分层（`docker-compose.yml` 生产语义 + `docker-compose.override.yml` 研发层，代码挂载/热更新/vhost 目录按需叠加）
> * 本地多项目接入：`docker/nginx/conf.d/` vhost 目录整体挂载，nginx 自动加载，新项目零改 compose
> * `.dockerignore` 构建上下文裁剪：本地项目（如 greenchina，557M vendor）、`.env` 凭据、node_modules 绝不进镜像
> * `APP_ENV` 环境参数（dev/prod），运行时行为与代码挂载职责分离
>
> 原规划为 v0.7.0 的「组件化与可配置栈」将顺延为 v0.7.1+，在本地研发模式之上继续参数化组件版本与 PHP 扩展。
